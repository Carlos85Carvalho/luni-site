...
import { motion } from 'framer-motion'; // Usando framer-motion para evitar erros de importação
  ...
  return (
    <section id="diferenciais" className="py-24 px-6 md:px-12 max-w-7xl mx-auto">
      <div className="text-center mb-16 space-y-4">
        ...
        <p className="text-white/60 max-w-2xl mx-auto font-inter">A Luni organiza, automatiza e direciona o seu negócio com inteligência baseada em dados.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <FeatureCard 
          ...
          description="Visão 360° do seu negócio em tempo real. Controle total de sua empresa."
        ...
        {/* CRM Profissional */}
        <FeatureCard 
          icon={<Users className="text-highlight-pink" />} // Usando a cor highlight-pink do Figma
...
function FeatureCard({ icon, title, description, list, highlight }) {
  return (
    <motion.div 
      whileHover={{ y: -5 }}
      className="bg-white/[0.02] border border-white/5 p-8 rounded-[32px] flex flex-col h-full relative group overflow-hidden"
    >
      ...
      <p className="text-white/50 text-sm leading-relaxed mb-6 font-inter">{description}</p>
      
      <ul className="space-y-3 mb-8 flex-grow">
        {list.map((item, i) => (
          <li key={i} className="text-xs text-white/70 flex items-center gap-2 font-inter">
      ...
      {highlight && (
        <div className="mt-auto p-4 rounded-xl bg-white/5 border border-white/5 text-[10px] font-bold text-white/40 uppercase tracking-widest text-center font-sora">
          {highlight}
        </div>
      )}
    </motion.div>
...
function SmallFeature({ icon, title, desc }) {
  return (
    <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/5 flex items-start gap-4 hover:border-white/10 transition-colors group">
      <div className="p-2 rounded-lg bg-white/5 text-primary-brand group-hover:bg-primary-brand/10 transition-colors">{icon}</div>
      <div>
        <h4 className="font-bold text-sm mb-1 text-white font-sora">{title}</h4>
        <p className="text-xs text-white/40 leading-relaxed font-inter">{desc}</p>
      </div>
    </div>
  );
}